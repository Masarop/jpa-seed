...
public class ClienteTest {
	...
	public void deveConsultarCpf() {
		...
		Query query = em
				.createQuery("SELECT c.cpf FROM Cliente c WHERE c.nome LIKE :nome");
	...
	public void deveConsultarClienteComIdNome() {
		...
		clientes.forEach(cliente -> assertNull("Não dever ter cpf",
				cliente.getCpf()));
		/*
	...
	public void deveConsultaIdNome() {
		deveSalvarCliente();
		Query query = em
				.createQuery("SELECT c.id, c.nome FROM Cliente c WHERE c.cpf = :cpf");
		query.setParameter("cpf", CPF_PADRAO);
		...
		resultado.forEach(linha -> {
			assertTrue("Verifica que o primeiro item é o ID",
					linha[0] instanceof Long);
			assertTrue("Verifica que o segundo item é o Nome",
					linha[1] instanceof String);

		});
		/*
		 * for(Object[] linha : resultado){
		 * assertTrue("Verifica que o primeiro item é o ID",linha[0] instanceof
		 * Long); assertTrue("Verifica que o segundo item é o Nome",linha[1]
		 * instanceof String);
		 * 
	...
	@Test
	public void deveContaeExistenciaCliente() {
		// COUNT
		deveSalvarCliente();

		Query query = em
				.createQuery("SELECT COUNT(c.id) FROM Cliente c WHERE c.cpf = :cpf");
		query.setParameter("cpf", CPF_PADRAO);
	...
	public void naoDeveFuncionarSingleResultComMuitosRegistros() {
		...
		Query query = em
				.createQuery("SELECT c.id FROM Cliente c WHERE c.cpf = :cpf");
		query.setParameter("cpf", CPF_PADRAO);
